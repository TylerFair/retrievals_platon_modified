...
class CombinedRetriever:
    ...
    def run_nautilus(self, transit_bins, transit_depths, transit_errors,
                     ...
                     fit_info,
                     include_condensation=True, rad_method="xsec",
                     n_live=250, n_eff=1000,
                     num_final_samples=1000, zero_opacities=[],
                     pool=None,
                     **nautilus_kwargs):
        """Runs Nautilus nested sampling to retrieve atmospheric parameters."""
        import nautilus
        ...
        if "nlive" in nautilus_kwargs and "n_live" not in nautilus_kwargs:
            n_live = nautilus_kwargs.pop("nlive")
        if "n_ess" in nautilus_kwargs and "n_eff" not in nautilus_kwargs:
            n_eff = nautilus_kwargs.pop("n_ess")
        if "n_effective" in nautilus_kwargs and "n_eff" not in nautilus_kwargs:
            n_eff = nautilus_kwargs.pop("n_effective")
        if "n_total" in nautilus_kwargs and "n_like_max" not in nautilus_kwargs:
            nautilus_kwargs["n_like_max"] = nautilus_kwargs.pop("n_total")

        requested_vectorized = bool(nautilus_kwargs.pop("vectorized", False))
        if "vectorize" in nautilus_kwargs:
            requested_vectorized = bool(nautilus_kwargs.pop("vectorize"))
        if "vectoried" in nautilus_kwargs:
            requested_vectorized = bool(nautilus_kwargs.pop("vectoried"))
        vectorized_likelihood_chunk = int(nautilus_kwargs.pop("vectorized_likelihood_chunk", 0) or 0)
        likelihood_log_every = int(nautilus_kwargs.pop("likelihood_log_every", 0) or 0)
        equal_weight_boost = float(nautilus_kwargs.pop("equal_weight_boost", 1.0))
        likelihood_eval_count = 0

        run_option_keys = (
            "f_live", "n_shell", "n_eff", "n_like_max",
            "discard_exploration", "timeout", "verbose"
        )
        run_options = {}
        for key in run_option_keys:
            if key in nautilus_kwargs:
                run_options[key] = nautilus_kwargs.pop(key)
        run_options.setdefault("n_eff", n_eff)

        num_dim = fit_info._get_num_fit_params()

        def nautilus_prior_transform(unit_cube):
            unit_cube = np.asarray(unit_cube, dtype=float)
            if unit_cube.ndim == 1:
                transformed = np.empty(num_dim, dtype=float)
                for i in range(num_dim):
                    transformed[i] = fit_info._from_unit_interval(i, unit_cube[i])
                return transformed
            if unit_cube.ndim == 2:
                transformed = np.empty_like(unit_cube, dtype=float)
                for row in range(unit_cube.shape[0]):
                    for i in range(num_dim):
                        transformed[row, i] = fit_info._from_unit_interval(i, unit_cube[row, i])
                return transformed
            raise ValueError(
                "Unexpected unit-cube shape for Nautilus prior transform: {}".format(
                    unit_cube.shape))

        def nautilus_ln_like_single(params):
            nonlocal likelihood_eval_count
            ln_like = self._ln_like(
                params, transit_calc, eclipse_calc, fit_info,
                transit_depths, transit_errors,
                eclipse_depths, eclipse_errors,
                zero_opacities=zero_opacities, lnlike_per_point=False)
            if not np.isfinite(ln_like) or ln_like < -1e30:
                ln_like = -1e30

            likelihood_eval_count += 1
            if likelihood_log_every > 0 and likelihood_eval_count % likelihood_log_every == 0:
                print("\nEvaluated params: {}".format(self.pretty_print(fit_info)))
            return float(ln_like)

        def nautilus_ln_like_vectorized(params):
            params = np.atleast_2d(params)
            lnlikes = np.empty(len(params), dtype=float)
            if len(params) == 0:
                return lnlikes
            if vectorized_likelihood_chunk <= 0:
                chunk_size = len(params)
            else:
                chunk_size = max(1, vectorized_likelihood_chunk)
            for start in range(0, len(params), chunk_size):
                stop = min(start + chunk_size, len(params))
                for i in range(start, stop):
                    lnlikes[i] = nautilus_ln_like_single(params[i])
            return lnlikes

        nautilus_likelihood = nautilus_ln_like_vectorized if requested_vectorized else nautilus_ln_like_single

        sampler_init_params = inspect.signature(nautilus.Sampler.__init__).parameters
        sampler_accepts_kwargs = any(
            param.kind == inspect.Parameter.VAR_KEYWORD
            for param in sampler_init_params.values())
        sampler_param_names = {name for name in sampler_init_params if name != "self"}

        sampler_kwargs = dict(nautilus_kwargs)
        sampler_kwargs["prior"] = nautilus_prior_transform
        sampler_kwargs["likelihood"] = nautilus_likelihood
        if "n_dim" in sampler_param_names:
            sampler_kwargs["n_dim"] = num_dim
        if "n_live" in sampler_param_names:
            sampler_kwargs["n_live"] = n_live
        elif "nlive" in sampler_param_names:
            sampler_kwargs["nlive"] = n_live

        if "vectorized" in sampler_param_names:
            sampler_kwargs["vectorized"] = requested_vectorized
        elif "vectorize" in sampler_param_names:
            sampler_kwargs["vectorize"] = requested_vectorized
        elif requested_vectorized:
            print("[nautilus] WARNING: sampler does not accept vectorized mode; falling back to scalar likelihood API")
            requested_vectorized = False
            sampler_kwargs["likelihood"] = nautilus_ln_like_single

        if "pass_dict" in sampler_param_names and "pass_dict" not in sampler_kwargs:
            sampler_kwargs["pass_dict"] = False

        if pool is not None and ("pool" in sampler_param_names or sampler_accepts_kwargs):
            sampler_kwargs["pool"] = pool

        if not sampler_accepts_kwargs:
            unsupported_keys = [key for key in sampler_kwargs if key not in sampler_param_names]
            if len(unsupported_keys) > 0:
                raise TypeError(
                    "Unsupported Nautilus Sampler kwargs for installed version: {}".format(
                        ", ".join(sorted(unsupported_keys))))

        sampler = nautilus.Sampler(**sampler_kwargs)

        run_params = inspect.signature(sampler.run).parameters
        run_accepts_kwargs = any(
            param.kind == inspect.Parameter.VAR_KEYWORD
            for param in run_params.values())
        run_param_names = set(run_params)
        run_kwargs = {}
        for key, value in run_options.items():
            if value is None:
                continue
            if key in run_param_names or run_accepts_kwargs:
                run_kwargs[key] = value

        resolved_n_eff = run_kwargs.get("n_eff", "default")
        print(
            "[nautilus] config: n_live={}, vectorized={}, n_eff={}, n_like_max={}, n_batch={}, n_networks={}".format(
                n_live,
                requested_vectorized,
                resolved_n_eff,
                run_kwargs.get("n_like_max", "default"),
                sampler_kwargs.get("n_batch", "default"),
                sampler_kwargs.get("n_networks", "default"),
            )
        )
        ...
        samples = np.asarray(posterior[0])
        logw = np.asarray(posterior[1], dtype=float)
        logl = np.asarray(posterior[2], dtype=float)
        linear_weights = np.exp(logw - np.max(logw))

        best_params_arr = samples[np.argmax(logl)]
        try:
            posterior_equal = sampler.posterior(
                equal_weight=True, equal_weight_boost=equal_weight_boost)
            equal_samples = np.array(posterior_equal[0], copy=True)
        except TypeError:
            weight_sum = np.sum(linear_weights)
            if np.isfinite(weight_sum) and weight_sum > 0:
                probs = linear_weights / weight_sum
                indices = np.random.choice(len(samples), size=len(samples), replace=True, p=probs)
                equal_samples = np.array(samples[indices], copy=True)
            else:
                equal_samples = np.array(samples, copy=True)
        equal_samples = np.atleast_2d(equal_samples)
        np.random.shuffle(equal_samples)

        logz = getattr(sampler, "log_z", None)
        if callable(logz):
            logz = logz()
        logz_err = getattr(sampler, "log_z_err", None)
        n_eff_est = getattr(sampler, "n_eff", None)
        if logz is None:
            logz_for_output = np.nan
        else:
            logz_for_output = float(logz)
        ...
        retrieval_result = RetrievalResult(
            nautilus_results, "nautilus", best_params_arr,
            transit_bins, transit_depths, transit_errors,
            eclipse_bins, eclipse_depths, eclipse_errors,
            best_fit_transit_depths, best_fit_transit_info,
            best_fit_eclipse_depths, best_fit_eclipse_info,
        ...
        for params in equal_samples[:num_final_samples]:
            ...
            if transit_depths is not None:
                ...
                retrieval_result.random_TP_profiles.append(
                    np.array([transit_info["P_profile"], transit_info["T_profile"]]))
            if eclipse_depths is not None:
                retrieval_result.random_eclipse_depths.append(eclipse_info["unbinned_eclipse_depths"])
                retrieval_result.random_TP_profiles.append(
